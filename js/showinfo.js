function showinfo(city){
    ...
    for (let i = 0; i < cities.length; i++){
        if (city===cities[i]){
            document.getElementById(logos[i]).style.opacity = '0';
            setTimeout( function(){document.getElementById(logos[i]).style.display = 'none';
            document.getElementById(infos[i]).style.display = 'block';}, 400);
            document.getElementById(infos[i]).style.opacity = '1';
        }
    }
}
...
function closeinfo(logo_id){
    ...
    for(let i = 0; i < buttons.length; i++){
        if(logo_id===buttons[i]){
            document.getElementById(infos[i]).style.opacity = '0';            
            setTimeout(function() {document.getElementById(logos[i]).style.display = 'block';
            document.getElementById(infos[i]).style.display = 'none';}, 400);
            document.getElementById(logos[i]).style.opacity = '1';
        }
    }
}